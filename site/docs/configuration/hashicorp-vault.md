...
A few additional notes about connecting the scraper to a CDB. These are example commands; adjust them to fit your environment.

When setting up a connection to a CDB, edit the `username_template` parameter so Vault creates a `C##` common user for the scraper.

```sh
vault write database/config/mydb \
    plugin_name=vault-plugin-database-oracle \
    allowed_roles="mydb_scraper" \
...
```sql
CREATE USER c##scraper NO AUTHENTICATION;
GRANT create session TO c##scraper;
GRANT all necessary privileges that the scraper needs TO c##scraper;
```

Create database role in Vault:

```sh
vault write database/roles/mydb_scraper \
    db_name=mydb \
    creation_statements='CREATE USER {{username}} IDENTIFIED BY "{{password}}"; GRANT CREATE SESSION TO {{username}}; ALTER USER c##scraper GRANT CONNECT THROUGH {{username}};' \
    default_ttl="7d" \
    max_ttl="10d"
```

Make sure to restart the scraper before the TTL above expires so it fetches new database credentials. When the TTL expires, Vault will drop the dynamically created database users.

Then create the database config in the scraper:
  ...
  mydb:
    vault:
      hashicorp:
        ...
        mountType: database
        mountName: database
        secretPath: mydb_scraper
        useAsProxyFor: c##scraper
```
